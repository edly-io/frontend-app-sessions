...
const { getCourseSessionsList, getSessionsPage } = require('./api');
...
beforeEach(() => {
  ...
  getSessionsPage.mockResolvedValue({ count: 0, results: [] });
});
...
it('shows the empty state when the course has no sessions', async () => {
  ...
  await selectCourse();
  await waitFor(() => expect(
    screen.getByText(/no completed sessions found for this course/i),
  ).toBeInTheDocument());
});
