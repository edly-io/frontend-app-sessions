...
const TraineeDashboardPage = ({ profileSwitcher = null, asTab = false, initialProgramKey = undefined }) => {
  const intl = useIntl();
  ...
  const scrollToFeedback = useCallback(() => {
    document.getElementById('trainee-feedback-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);
  ...
  const showProgramDropdown = !asTab && !initialProgramKey && data.available_programs.length > 1;
  ...
  return wrap(
    <>
      ...
      <DashboardHero
        ...
        programKey={data.programme?.program_key}
      />
...
TraineeDashboardPage.propTypes = {
  ...
  initialProgramKey: PropTypes.string,
};
